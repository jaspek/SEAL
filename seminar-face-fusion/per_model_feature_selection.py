"""
Per-model feature selection — applies the same selection methods as
feature_selection.py, but to each 512-d single-model embedding separately,
to answer: 'Does the 6x dimensional redundancy hold for individual models, or
only for the fused embedding?'
"""
import numpy as np
import warnings
...
def evaluate_fs(method_fn, name, emb, pair_idx, k_values):
    ...
    return {k: (np.mean(results[k]), np.std(results[k])) for k in k_values}
...
def main():
    ...
    print(f"  pairs: {len(pair_idx)}\n")

    # L2-normalize each (should already be normalized but cheap to re-do)
    def norm(e):
        return e / (np.linalg.norm(e, axis=1, keepdims=True) + 1e-10)

    embeddings = [
        ("ArcFace", norm(data["arcface"])),
        ("MagFace", norm(data["magface"])),
        ("AdaFace", norm(data["adaface"])),
    ]

    K_VALUES = [32, 64, 128, 256, 384, 512]
    ...
    # Run per-model
    all_results = {}  # {model_name: {method_name: {k: (mean, std)}}}
    for model_name, emb in embeddings:
        print(f"=== {model_name} ===")
        per_method = {}
        for fn, name in methods:
            per_method[name] = evaluate_fs(fn, name, emb, pair_idx, K_VALUES)
            row = "  " + name.ljust(25) + " | " + " | ".join(
                f"{per_method[name][k][0]*100:6.2f}" for k in K_VALUES)
            print(row)
        per_method["PCA"] = evaluate_pca(emb, pair_idx, K_VALUES)
        row = "  " + "PCA".ljust(25) + " | " + " | ".join(
            f"{per_method['PCA'][k][0]*100:6.2f}" for k in K_VALUES)
        print(row)
        all_results[model_name] = per_method
        print()

    # Final summary tables, one per model
    print("\n\n=== Summary Tables (mean accuracy %) ===")
    for model_name, per_method in all_results.items():
        print(f"\n--- {model_name} ---")
        header = f"  {'Method':25s} | " + " | ".join(f"k={k:4d}" for k in K_VALUES)
        print(header); print("-" * len(header))
        for method_name, res in per_method.items():
            row = f"  {method_name:25s} | " + " | ".join(
                f"{res[k][0]*100:6.2f}" for k in K_VALUES)
            print(row)
