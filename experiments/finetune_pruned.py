...
from run_pruning import prune_magface, load_data          # noqa: E402
...
def _prep(img):
    ...
    return torch.from_numpy(M._pre_magface(img))
...
class RecFile(Dataset):
    ...
    def __init__(self, rec_path, idx_path, limit=None, seed=0):
...
class FaceFolder(Dataset):
    ...
    def __init__(self, root, limit=None, seed=0):
    ...
    def __getitem__(self, i):
        return _prep(cv2.imread(str(self.files[i])))


def build_dataset(root, limit=None, seed=0):
    """Folder of images, a .rec file, or a folder containing train.rec."""
    root = Path(root)
    if root.suffix == ".rec":
        return RecFile(root, root.with_suffix(".idx"), limit, seed)
    recs = sorted(root.rglob("*.rec"))
    if recs:
        rec = next((r for r in recs if r.stem == "train"), recs[0])
        print(f"found RecordIO pack: {rec}")
        return RecFile(rec, rec.with_suffix(".idx"), limit, seed)
    return FaceFolder(root, limit, seed)
...
def main():
    ...
    ap.add_argument("--out", default="pruning_finetuned.csv")
    args = ap.parse_args()
    ...
    ds = build_dataset(root, args.limit, args.seed)
    if len(ds) == 0:
    ...
    teacher = M.load_magface().eval()
    for p in teacher.parameters():
        p.requires_grad_(False)

    rows = []
    for ratio in args.ratios:
        print(f"\n=== prune {args.criterion} @ {ratio:.0%} "
              f"+ distill {args.epochs} epoch(s) ===")
        student, bmacs, bparams, macs, params = prune_magface(ratio, args.criterion)
